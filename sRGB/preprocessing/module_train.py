...
class TrainModule(object):
    ...
    def __init__(self, cuda_num, DataParallel=False, edge_lambda=1, color_lambda=1, sigma=4):
        ...
        self.L1 = nn.L1Loss()
        self.Edge_L = module_losses.EdgeLoss(cuda_num=cuda_num)
        self.Color_L = module_losses.ColorLoss(cuda_num=cuda_num, sigma=sigma)
    ...
    def weight_loader_netG(self, load_checkpoint_dir):
        # 이 함수에서 checkpoint
    ...
    def optimize_parameters(self, batch):
        ...
        ##
        target_fake = self.netG(input, target_clone)
        ...
        self.logs_dict['err_edge'] = err_edge.item()
        self.logs_dict['err_color'] = err_color.item()
