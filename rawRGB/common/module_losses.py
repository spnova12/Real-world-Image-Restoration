...
class ColorLoss(nn.Module):
    def __init__(self, cuda_num, sigma=4, with_flat_raw=False):
        super(ColorLoss, self).__init__()
        self.with_flat_raw = with_flat_raw
    ...
    def conv_guss_with_sigma(self, img):
        if self.with_flat_raw:
            print('hi')
            normalized_raw = img
            cfa_data = torch.zeros(img.shape[0], 1, img.shape[2]*2, img.shape[3]*2).to(self.device)
            cfa_data[:, :, 0::2, 0::2] = normalized_raw[:, 0:1, :, :]
            cfa_data[:, :, 0::2, 1::2] = normalized_raw[:, 1:2, :, :]
            cfa_data[:, :, 1::2, 0::2] = normalized_raw[:, 2:3, :, :]
            cfa_data[:, :, 1::2, 1::2] = normalized_raw[:, 3:4, :, :]
            img = cfa_data
    ...
    def forward(self, x, y):
        loss = self.loss(self.conv_guss_with_sigma(x),
                         self.conv_guss_with_sigma(y))
        return loss
...
if __name__ == "__main__":
    ...
    # get kernel
    # Since we are calling correlate, not convolve, revert the kernel
    weights = _gaussian_kernel1d(sigma=10)[::-1]
    k = torch.Tensor([weights])
